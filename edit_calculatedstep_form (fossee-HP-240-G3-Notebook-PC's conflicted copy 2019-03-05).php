...
/**
 * Editing form for the calculated step question type.
 *
 * @copyright  2007 Jamie Pratt me@jamiep.org
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_calculatedstep_edit_form extends qtype_calculated_edit_form {
    /**
     * Handle to the question type for this question.
     *
     * @var question_calculatedstep_qtype
     */
    public $qtypeobj;
    ...
    public function __construct($submiturl, $question, $category, $contexts, $formeditable = true) {
        ...
        if (optional_param('addbutton', false, PARAM_BOOL)) {
            ...
            if (optional_param_array('definition', '', PARAM_NOTAGS) &&
                ...
                echo '<br><br><br>==============fromformnumber bef===============';
                print_object($fromformnumber);
...
//                     if($defid == "1-0-scadans1") {
//                         $itemnumber = ceil($i / count($this->datasetdefs));
...
//         }
    ...
    protected function definition_inner($mform) {
        ...
        $label = "<div class='mdl-align'></div><div class='mdl-align'>" .
                get_string('wildcardrole', 'qtype_calculatedstep') . "</div>";
        ...
        $mform->addElement('submit', 'analyzequestion',
                get_string('findwildcards', 'qtype_calculatedstep'));
        ...
        if (!empty($this->datasetdefs)) {// So there are some datadefs.
            ...
            $mform->addElement('header', 'additemhdr',
                    get_string('wildcardparam', 'qtype_calculatedstep'));
            $idx = 1;
...
//                                 $datasetitem->value = $scadansvalue;
            ...
            echo '<br>===========ln 541===formdata==================';
            print_object($this->formdata);
            ...
            $addgrp[] = $mform->createElement('submit', 'addbutton',
                    get_string('generatenewitemsset', 'qtype_calculatedstep'));
            $addgrp[] = $mform->createElement('select', "selectadd", '', $addoptions);
            $addgrp[] = $mform->createElement('static', "stat", '',
                    get_string('newsetwildcardvalues', 'qtype_calculatedstep'));
            ...
            if ($this->noofitems == 0) {
                $mform->addElement('static', 'warningnoitems', '', '<span class="error">' .
                        get_string('youmustaddatleastonevalue', 'qtype_calculatedstep') .
                        '</span>');
                $mform->closeHeaderBefore('warningnoitems');
            } else {
                $mform->addElement('header', 'additemhdr1',
                        get_string('wildcardvalues', 'qtype_calculatedstep'));
                ...
                if (!empty($this->numbererrors) || $this->outsidelimit) {
                    $mform->addElement('static', "alert", '', '<span class="error">' .
                            get_string('useadvance', 'qtype_calculatedstep').'</span>');
                }

                $mform->addElement('submit', 'updatedatasets',
                        get_string('updatewildcardvalues', 'qtype_calculatedstep'));
                ...
                for ($i = $this->noofitems; $i >= 1; $i--) {
                    foreach ($this->datasetdefs as $defkey => $datasetdef) {
                        ...
                        if ($k > 0 ||  $this->outsidelimit || !empty($this->numbererrors)) {
                            $mform->addElement('text', "number[{$j}]", get_string(
                                    'wildcard', 'qtype_calculatedstep', $datasetdef->name));
                    ...
                    if (!empty($strquestionlabel) && ($k > 0 ||  $this->outsidelimit ||
                        ...
                        $mform->addElement('static', "answercomment[{$i}]", "<b>" .
                                get_string('setno', 'qtype_calculatedstep', $i) .
                                "</b>&nbsp;&nbsp;" . $strquestionlabel);
            ...
            $mform->addElement('static', 'warningnowildcards', '', '<span class="error">' .
                    get_string('atleastonewildcard', 'qtype_calculatedstep') . '</span>');
        ...
        if (!empty($this->question->id)) {
            if ($this->question->formoptions->cansaveasnew) {
                $mform->addElement('header', 'additemhdr',
                        get_string('converttocalculated', 'qtype_calculatedstep'));
                $mform->closeHeaderBefore('additemhdr');

                $mform->addElement('checkbox', 'convert', '',
                        get_string('willconverttocalculated', 'qtype_calculatedstep'));
    ...
    public function validation($data, $files) {
        ...
        if (empty($data['definition'])) {
            $errors['selectadd'] = get_string('youmustaddatleastonevalue', 'qtype_calculatedstep');
        }
